...
def import_data():
    ...
    if len(os.listdir('C:\\Users\\a\\Desktop\\Data load python\\Packing files') ) == 0:
        print('"Packing files" Folder is empty.')
        df = pd.DataFrame()
        return df
    else:
        stock_files = sorted(glob('C:\\Users\\a\\Desktop\\Data load python\\Packing files\\*.csv'))
...
def load_postgreSQL(df):
    ...
    try:
        conn = psycopg2.connect(
            host = "database1.csioilzjaaf3.ap-south-1.rds.amazonaws.com",
            dbname = "database1",
            user = "admin",
            password = "dabba@123",
            port = 5432
